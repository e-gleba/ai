...
code: reproduce the problem with public APIs and generic names, or run a local
model — [local_models](local_models.md). Write the off-limits directories into the
project instructions so an agent cannot wander in —
...
Confirm the generated code on [Compiler Explorer](https://godbolt.org) and the
effect on [Quick Bench](https://quick-bench.com). A performance claim without a
number is not a result — [failure_modes](failure_modes.md).
