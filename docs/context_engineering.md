...
Keep it under about 100 lines; it is read every session, and some tools cap the size
they will load.
...
Rule or skill? A rule is a constraint that is always true. A skill is a procedure for
a class of task. If it has numbered steps, it is a skill. The `description` decides
whether it loads, so write it as "what it does and when to use it".
