...
the [working draft](https://eel.is/c++draft/) when the exact wording matters,
...
1. **Value semantics first.** A type that copies correctly, moves cheaply, and has
   no hidden sharing is almost impossible to misuse. Reach for references, pointers,
   and shared ownership only when the design requires them. What a well-behaved type
   owes its users is set out in *The C++ Programming Language* by Bjarne Stroustrup
   [book](https://www.stroustrup.com/4th.html) and argued case by case in the
   [C++ Core Guidelines](https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines).

2. **Algorithms over loops, concepts over types.** State the requirement, not the
   type. This is the tradition the standard library came from: generic programming as
   developed by Alexander Stepanov, from first principles in *Elements of
   Programming* [book](http://elementsofprogramming.com) and *From Mathematics to
   Generic Programming*.

3. **Zero overhead means measured, not assumed.** An abstraction is free only if the
   generated code says so. Practical small-utility design in that spirit is what
   [Boost](https://www.boost.org) exists for; the work of its maintainers — for
   example Antony Polukhin's `Boost.PFR`, `Boost.Stacktrace` and `Boost.DLL`
   [libraries](https://github.com/apolukhin) — shows how much is possible with no
   runtime cost.
...
- In a hot path, compare the generated code against a plain loop on
  [Compiler Explorer](https://godbolt.org) and measure on
  [Quick Bench](https://quick-bench.com). Readability wins outside hot paths and
  loses inside them when the numbers say so.
...
Concepts make the error message point at the caller's mistake instead of at line 900
of a header. Name them after the requirement, not after the types that satisfy it.

## what to be careful with

| facility | caution |
| --- | --- |
| coroutines | the language gives the machinery, not a scheduler; frame allocation and lifetime are yours |
| modules | toolchain and build-system support still varies; check before committing a project to them |
| `std::shared_ptr` | shared ownership is a design decision, not a convenience; cycles and atomic refcount traffic cost real time |
| exceptions | fine inside a module, dangerous across binary boundaries and plugin loads |
| `std::function` | type erasure allocates and hides an indirect call; in hot paths use a concrete callable |
...
Answer with a source, a compiler-support check, and generated code. Fashion is not an
argument; neither is a blog post. If the standard facility is heavier than the
